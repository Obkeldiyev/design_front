...
import { CARD_TEMPLATES, TEMPLATE_CATEGORIES, type CardTemplate } from "@/lib/card-templates";
...
function NewDesign() {
  ...
  const filteredTemplates =
    categoryFilter === "All"
      ? CARD_TEMPLATES
      : CARD_TEMPLATES.filter((t) => t.category === categoryFilter);

  const buildDocFromTemplate = (template?: CardTemplate | null): CanvasDoc => {
    if (template) return JSON.parse(JSON.stringify(template.doc));
    const p = SIZE_PRESETS.find((x) => x.id === preset) ?? SIZE_PRESETS[0];
    return {
      version: 1,
      canvas: { width: p.w, height: p.h, background: "#ffffff" },
      pages: [{ id: generateId(), name: "Front", fabric: { version: "6.0.0", objects: [] } }],
    };
  };

  const create = useMutation({
    mutationFn: (templateOverride?: CardTemplate | null) => {
      const doc = buildDocFromTemplate(templateOverride ?? selectedTemplate);
      return DesignAPI.create({
        title,
        slug: slugify(title || "untitled-design"),
        type,
        businessId: businessId ?? null,
        data: doc,
      } as Parameters<typeof DesignAPI.create>[0]);
    },
    onSuccess: (d) => {
      ...
      <div className="flex flex-col bg-background" style={{ height: "100vh", overflow: "hidden" }}>
        <div className="flex items-center justify-between border-b border-border bg-card px-6 py-4 flex-shrink-0">
          <div className="flex items-center gap-3">
            <Link to="/designs" className="text-muted-foreground hover:text-foreground">
              ...
              return (
                ...
                >
                  <div className="relative overflow-hidden">
                    ...
                    <div
                      className={`absolute inset-0 flex items-center justify-center bg-background/70 backdrop-blur-sm transition-opacity ${isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
                    >
                      <Button
                        type="button"
                        size="sm"
                        ...
                        className="shadow-lg"
                      >
                        {create.isPending ? t("designs.creating") : t("designs.use_template")}
                      </Button>
                    </div>
                    {isSelected && (
                      <div className="absolute top-2 left-2 bg-primary rounded-full p-1.5 shadow">
                        <Check className="h-4 w-4 text-primary-foreground" />
                      </div>
                    )}
                    {template.isPremium && (
                      <div className="absolute top-2 right-2 flex items-center gap-1 bg-amber-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">
      ...
      <div className="flex items-start gap-6 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold">
            {selectedTemplate ? selectedTemplate.title : t("designs.blank_canvas")}
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            {selectedTemplate
              ? `${selectedTemplate.category} — ${selectedTemplate.width}×${selectedTemplate.height}px`
              : t("designs.blank_desc")}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-5 rounded-2xl border border-border bg-card p-6">
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder={t("designs.design_title_placeholder")}
          />
        </div>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(v) => setBusinessId(v === "none" ? undefined : v)}
          >
            <SelectTrigger>
              <SelectValue placeholder={t("designs.link_business")} />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="flex justify-end pt-2">
          <Button onClick={() => create.mutate(null)} disabled={create.isPending} size="lg">
