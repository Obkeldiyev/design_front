...
const SHADOWS: Record<string, string> = {
  ...
  lifted: "0 18px 50px rgba(15, 23, 42, 0.14)",
  strong: "0 28px 80px rgba(15, 23, 42, 0.22)",
};

function cssPx(value: unknown, fallback: number) {
  ...
  return {
    paddingY:
      type === "navbar"
        ...
        : type === "footer"
          ? 56
          : type === "text" || type === "gallery"
            ? 64
            : 80,
    paddingX: 32,
    itemRadius: type === "team" ? 999 : type === "navbar" ? 8 : 16,
    imageRadius: type === "team" || type === "testimonials" ? 999 : 16,
    buttonRadius: 12,
  };
}

function blockShellStyle(block: Block): React.CSSProperties {
  const content = block.content;
  const defaults = designDefaults(block.type);
  const animation = String(content.animation || "none");
  const style = {
    "--site-py": cssPx(content.paddingY, defaults.paddingY),
    "--site-px": cssPx(content.paddingX, defaults.paddingX),
    "--site-card-radius": cssPx(content.itemRadius, defaults.itemRadius),
    "--site-image-radius": cssPx(content.imageRadius, defaults.imageRadius),
    "--site-button-radius": cssPx(content.buttonRadius, defaults.buttonRadius),
  ...
  if (animation !== "none") {
    style.animationName = `card24-${animation}`;
    style.animationDuration = "620ms";
    style.animationDelay = `${Number(content.animationDelay || 0)}ms`;
...
function EditText({
  ...
  return (
    <Tag
      ...
      className={[
        className,
        "outline-none cursor-text rounded-sm",
...
function ImageUpload({
  ...
  if (!editable) {
    return src ? (
      ...
      <div
        className={`${className} bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center text-slate-400 text-sm font-medium`}
      >
        Image
      </div>
    );
  ...
  return (
    <div
      className={`${className} relative group/img cursor-pointer`}
      onClick={(e) => {
        e.stopPropagation();
...
function DraggableItem({
  ...
  return (
    ...
    >
      ...
      {/* Item toolbar */}
      <div
        className="absolute top-1 right-1 z-20 hidden group-hover/item:flex items-center gap-0.5 bg-gray-900/80 text-white rounded-lg px-1 py-0.5 shadow-lg"
        onClick={(e) => e.stopPropagation()}
        onMouseDown={(e) => e.stopPropagation()}
      >
        <span
          className="cursor-grab px-1 text-gray-400 hover:text-white text-xs"
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        {editable && !c.logoUrl && (
          <label
            className="cursor-pointer text-xs opacity-50 hover:opacity-100 border border-dashed border-current rounded px-1.5 py-0.5"
            onClick={(e) => e.stopPropagation()}
          >
            📷
            <input
              ...
              onChange={(e) => {
                ...
                r.onload = (ev) => upd({ logoUrl: ev.target?.result as string });
                r.readAsDataURL(f);
              }}
            />
          </label>
        ...
        {editable && c.logoUrl && (
          <button
            ...
            className="text-xs opacity-50 hover:opacity-100 text-red-500"
            title="Remove image"
          >
            ✕
          </button>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        {editable && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              setLinks([...(c.links || []), { label: "New Link", href: "#" }]);
            }}
            className="text-xs px-2 py-1 border border-dashed border-current rounded opacity-50 hover:opacity-100"
          >
            + Link
          </button>
        )}
...
function HeroBlock({ block, theme, editable, onUpdate }: BlockRendererProps) {
  ...
  const bgStyle: React.CSSProperties = c.backgroundImage
    ? {
        ...
        backgroundSize: c.backgroundFit || "cover",
  ...
  return (
    <section
      style={bgStyle}
      className="relative w-full min-h-[520px] flex flex-col justify-center px-8 py-20"
    >
      {editable && (
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2">
          <label
            className="cursor-pointer flex items-center gap-1 bg-black/50 text-white text-xs px-3 py-1.5 rounded-full hover:bg-black/70 transition-colors"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (!f) return;
                const r = new FileReader();
                r.onload = (ev) => upd({ backgroundImage: ev.target?.result as string });
                r.readAsDataURL(f);
              }}
            />
          </label>
      ...
      {(c.logoUrl || c.logoText || editable) && (
        <div className="absolute top-6 left-8 flex items-center gap-2">
          ...
          {editable && !c.logoUrl && (
            <label
              className="cursor-pointer text-xs opacity-50 hover:opacity-100 border border-dashed border-white rounded px-1.5 py-0.5 text-white"
              onClick={(e) => e.stopPropagation()}
            >
              📷
              <input
      ...
      <div className={`flex flex-col gap-5 max-w-3xl mx-auto w-full ${alignClass}`}>
        ...
        <div
          ...
          style={{
            justifyContent:
              c.layout === "center" ? "center" : c.layout === "right" ? "flex-end" : "flex-start",
          }}
        >
          {c.buttonText && (
            <span
          ...
          {c.secondaryButtonText && (
            <span className="px-7 py-3 rounded-lg font-semibold border-2 border-white/60 text-white inline-block">
              <EditText
                value={c.secondaryButtonText}
                onSave={(v) => upd({ secondaryButtonText: v })}
                editable={editable}
                className="font-semibold text-white"
          ...
          {editable && (
            <button
              onClick={(e) => {
                ...
                setItems([
                  ...(c.items || []),
                  { author: "New Person", role: "Role", text: "Great!", rating: 5, avatar: "" },
                ]);
              }}
        ...
        <div className="grid grid-cols-4 gap-8">
          ...
          {editable && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                setItems([...(c.items || []), { value: "0", label: "Metric" }]);
              }}
                  ...
                  {(plan.features || []).map((f: string, fi: number) => (
                    <li key={fi} className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <EditText
                        ...
                        onSave={(v) => {
                          const ps = [...c.plans];
                          const feats = [...ps[i].features];
                          feats[fi] = v;
                          ps[i] = { ...ps[i], features: feats };
                          setPlans(ps);
                        }}
                        editable={editable}
                        tag="span"
                      ...
                      {editable && (
                        <button
                          onClick={(e) => {
                            ...
                            ps[i] = {
                              ...ps[i],
                              features: plan.features.filter((_: any, ffi: number) => ffi !== fi),
                            };
                            setPlans(ps);
                          }}
                  ...
                  {editable && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        const ps = [...c.plans];
                        ps[i] = { ...ps[i], features: [...(ps[i].features || []), "New feature"] };
                        setPlans(ps);
                      }}
                      className="text-xs text-violet-500 hover:underline mt-1 text-left"
                    >
                      + Add feature
                    </button>
                  )}
  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto flex flex-col items-center gap-6">
        ...
        {(c.subtitle || editable) && (
          <p className="text-lg opacity-80 w-full">
            <EditText
              value={c.subtitle || ""}
              onSave={(v) => upd({ subtitle: v })}
              editable={editable}
        ...
        <div className="flex gap-3 flex-wrap justify-center">
          ...
          {c.secondaryButtonText && (
            <span className="px-8 py-3 border-2 border-white/60 rounded-lg font-semibold inline-block">
              <EditText
                value={c.secondaryButtonText}
                onSave={(v) => upd({ secondaryButtonText: v })}
                editable={editable}
    ...
    <section style={{ backgroundColor: c.backgroundColor || "#fff" }} className="w-full py-20 px-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="space-y-4">
          ...
          >
            <EditText
              value={c.buttonText || "Send Message"}
              onSave={(v) => upd({ buttonText: v })}
              editable={editable}
              tag="span"
              className="font-semibold text-white"
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          {c.videoUrl ? (
            ...
            <div className="w-full h-full bg-slate-800 flex items-center justify-center text-slate-400 text-6xl">
              ▶
            </div>
          )}
        </div>
...
function FooterBlock({ block, theme, editable, onUpdate }: BlockRendererProps) {
  ...
  return (
    <footer
      style={{ backgroundColor: c.backgroundColor || "#0f172a", color: c.textColor || "#94a3b8" }}
      className="w-full px-8 pt-16 pb-8"
    >
      <div className="max-w-6xl mx-auto">
        <div
          className="grid gap-12"
          style={{ gridTemplateColumns: `2fr ${(c.columns || []).map(() => "1fr").join(" ")}` }}
        >
          {/* Brand column */}
          <div>
            {c.logoUrl ? (
              ...
              <EditText
                ...
                editable={editable}
                tag="span"
                className="text-xl font-bold text-white block mb-3"
                placeholder="Brand"
              />
            )}
            <EditText
              value={c.tagline || ""}
              onSave={(v) => upd({ tagline: v })}
              editable={editable}
              tag="p"
                      ...
                      {editable && (
                        <button
                          onClick={(e) => {
                            ...
                            cols[ci] = {
                              ...cols[ci],
                              links: col.links.filter((_: any, i: number) => i !== li),
                            };
                            setCols(cols);
                          }}
                          className="opacity-0 group-hover/link:opacity-100 text-red-400 hover:text-red-300 text-xs ml-1"
                        >
                          ✕
                        </button>
                      )}
...
export function BlockRenderer({ block, theme, editable, onUpdate }: BlockRendererProps) {
  ...
  switch (block.type) {
    case "navbar":
      rendered = <NavbarBlock {...props} />;
      break;
    case "hero":
      rendered = <HeroBlock {...props} />;
      break;
    case "features":
      rendered = <FeaturesBlock {...props} />;
      break;
    case "testimonials":
      rendered = <TestimonialsBlock {...props} />;
      break;
    case "text":
      rendered = <TextBlock {...props} />;
      break;
    case "stats":
      rendered = <StatsBlock {...props} />;
      break;
    case "gallery":
      rendered = <GalleryBlock {...props} />;
      break;
    case "team":
      rendered = <TeamBlock {...props} />;
      break;
    case "pricing":
      rendered = <PricingBlock {...props} />;
      break;
    case "cta":
      rendered = <CtaBlock {...props} />;
      break;
    case "contact":
      rendered = <ContactBlock {...props} />;
      break;
    case "video":
      rendered = <VideoBlock {...props} />;
      break;
    case "footer":
      rendered = <FooterBlock {...props} />;
      break;
    default:
      rendered = (
        <section className="w-full py-12 px-8 bg-gray-50 text-center text-gray-400">
  ...
  return (
    <div className="card24-site-block" style={blockShellStyle(block)}>
      {rendered}
    </div>
  );
}
